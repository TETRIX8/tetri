    ...
    if message.from_user.id == admin_id:
        ...
        for user in datausers:
            try:
                ...
                await bot.send_message(message.chat.id, f'❗[ {user} ] — нежелательный пользователь.\n\nСоветуем удалить его из базы данных.')
                failed_count += 1
                pass
        await bot.send_message(message.chat.id, f"✅ Рассылка успешно окончена!\n"
...
async def delete_user(message: types.Message):
    if message.from_user.id == admin_id:
        ...
        if user_id_to_delete in datausers:
            datausers.remove(user_id_to_delete)
        ...
        await message.answer('❌ Вы не являетесь администратором данного бота.')
# Обработчик команды /удалитьнежелательных
@dp.message_handler(commands=['удалитьнежелательных'])
async def delete_unwanted_users(message: types.Message):
    if message.from_user.id == admin_id:
        unwanted_users = []
        for user in datausers:
            try:
                # Отправка тестового сообщения для проверки активности пользователя
                await bot.send_message(user, "Тестовое сообщение")
            except Exception as e:
                await bot.send_message(message.chat.id, f'❗[ {user} ] — нежелательный пользователь.nnСоветуем удалить его из базы данных.')
                unwanted_users.append(user)

        for user_id in unwanted_users:
            if user_id in datausers:
                datausers.remove(user_id)

        await message.answer(f"✅ Удаление нежелательных пользователей успешно завершено!n"
                             f"Количество удаленных пользователей: {len(unwanted_users)}")
    else:
        await message.answer('❌ Вы не являетесь администратором данного бота.')
...
async def send(call: types.CallbackQuery):
    ...
    async def handle_text(message: types.Message, state: FSMContext):
     ...
     play.add(types.InlineKeyboardButton(text="😍 Смотреть онлайн", url=f'https://film.kinozzz.com/play/?id={film_id}'),
     )
